...
async function main() {
  ...
  await db.delete(accessLogs).run();
  await db.delete(accessLogVehicles).run();
  await db.delete(workerDocuments).run();
  await db.delete(plannedAccessPersons).run();
  await db.delete(plannedAccesses).run();
  await db.delete(externalWorkers).run();
  await db.delete(users).run();
  await db.delete(workCategories).run();
  await db.delete(companies).run();
  await db.delete(departments).run();
  await db.delete(sites).run();

  process.stdout.write("Tables cleared\n");
  ...
  console.log("Seed completed. Admin: admin / demo123. Operator: operador / demo123");
}
