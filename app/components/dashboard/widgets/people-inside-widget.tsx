...
export function PeopleInsideWidget({
  scope,
  editMode,
}: WidgetComponentProps) {
  const def = WIDGET_REGISTRY["people-inside"];
  const { data, isLoading, revalidate } = useWidgetData<PeopleInsideData>(
    "/api/dashboard/people-inside",
    scope,
    def.refreshMs,
  );
    ...
    >
      {data === undefined ? (
        ...
        <Empty className="py-8">
          ...
          <EmptyDescription>
            No hay personas que se encuentren actualmente dentro de las
            instalaciones.
          </EmptyDescription>
        </Empty>
      ) : (
